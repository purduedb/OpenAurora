...
public:
     ...
     */
     void ReadBufferCommon(_Page& _return, const _Smgr_Relation& _reln, const int32_t _relpersistence, const int32_t _forknum, const int32_t _blknum, const int32_t _readBufferMode, const int64_t _lsn) {
        ...
        START_TIMING(&start);
#endif
#ifdef DEBUG_REPLAY_LATENCY
        uint64_t start_replay_lsn = 0;
        uint64_t end_replay_lsn = 0;
        uint64_t target_page_lsn = 0;

        start_replay_lsn = XLogParseUpto;
#endif
...
//        printf("%s %d start, _lsn = %lu, foundBasePage = %d, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u\n",
...
#ifdef DEBUG_REPLAY_LATENCY
            target_page_lsn = lsn;
            printf("%s, start_replay_lsn = %lu, end_replay_lsn = %lu, target_page_lsn = %lu\n", __func__ , start_replay_lsn, end_replay_lsn, target_page_lsn);
            fflush(stdout);
#endif
...
#ifdef DEBUG_TIMING
            RECORD_TIMING(&start, &end, "GetPageFromDisk");
#endif
...
//            fflush(stdout);
        ...
        GetPageFromRocksdb(bufferTag, lsn, &targetPage);
        ...
        free(targetPage);
#ifdef DEBUG_TIMING
        RECORD_TIMING(&start, &end, "GetPageFromRocksDB");
#endif
//        printf("%s %d end, _lsn = %lu\n", __func__ , __LINE__, _lsn);
//        fflush(stdout);
        return;
