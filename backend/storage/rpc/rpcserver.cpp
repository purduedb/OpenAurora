...
//#define DEBUG_PAGE_GRANULARITY 1
...
//        fflush(stdout);
        ...
        int count = 0;
        while(XLogParseUpto < targetLsn-1024 && !reachXlogTempEnd ) {
            if (count >= 500) {
...
void WaitReplay(uint64_t _lsn) {
#ifdef ENABLE_DEBUG_INFO
    printf("%s %d, parameter lsn = %lu, replayUpto = %lu, flushUpto = %lu\n", __func__ ,
           __LINE__, _lsn, XLogReplayUpto, RpcXLogFlushedLsn);
    fflush(stdout);
#endif
    if((XLogRecPtr)_lsn > XLogReplayUpto && XLogReplayUpto < RpcXLogFlushedLsn && XLogReplayUpto != 0) {
        ...
        int count = 0;
        while(XLogReplayUpto < targetLsn) {
#ifdef ENABLE_DEBUG_INFO
            printf("%s %d\n", __func__ , __LINE__);
            fflush(stdout);
#endif
            ...
            if (count >= 500) {
                printf("%s %d, clog lsn = %lu, target lsn = %lu, count = %d\n", __func__ ,
                       __LINE__, XLogReplayUpto, targetLsn, count);
                count = 0;
            } else {
                count ++;
            }
...
#ifdef ENABLE_DEBUG_INFO
        printf("%s %d, current ReplayUpto = %lu, parameter lsn = %lu\n", __func__ , __LINE__, XLogReplayUpto, _lsn);
        fflush(stdout);
#endif
    }
}
...
public:
     ...
     */
     void ReadBufferCommon(_Page& _return, const _Smgr_Relation& _reln, const int32_t _relpersistence, const int32_t _forknum, const int32_t _blknum, const int32_t _readBufferMode, const int64_t _lsn) {
        ...
        printf("%s, start, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u, lsn = %lu\n",
...
#ifdef DEBUG_TIMING
        RECORD_TIMING(&start, &end, "SearchHashMap");
#endif
        //! TODO PRINT THE LSN LIST.
        //! It seems that HashMap declare RocksDB has a version. However, indeed it is not.
        //! Or, Parse insert this lsn to HashMap, but RocksDB does not have this version (not replayed yet).

#ifdef ENABLE_DEBUG_INFO
        printf("%s %d , _lsn = %lu, foundBasePage = %d, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u\n",
               __func__ , __LINE__, _lsn, foundBasePage, _reln._spc_node, _reln._db_node, _reln._rel_node, _forknum, _blknum);
        fflush(stdout);
#endif
...
#ifdef DEBUG_PAGE_GRANULARITY
            printf("%s, end1, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u, lsn = %lu\n",
                   __func__ , _reln._spc_node, _reln._db_node, _reln._rel_node, _forknum, _blknum, _lsn);
            fflush(stdout);
#endif
#ifdef DEBUG_TIMING
            RECORD_TIMING(&start, &end, "ReadPageFromDisk");
#endif
#ifdef ENABLE_DEBUG_INFO
            printf("%s %d end\n", __func__ , __LINE__);
            fflush(stdout);
#endif
//            printf("%s %d end, _lsn = %lu\n", __func__ , __LINE__, _lsn);
//            fflush(stdout);
            return;
        }
...
#ifdef DEBUG_PAGE_GRANULARITY
        printf("%s, end2, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u, lsn = %lu\n",
               __func__ , _reln._spc_node, _reln._db_node, _reln._rel_node, _forknum, _blknum, _lsn);
        fflush(stdout);
#endif

#ifdef DEBUG_TIMING
        RECORD_TIMING(&start, &end, "WaitReplay");
#endif
#ifdef ENABLE_DEBUG_INFO
        printf("%s %d, targetLsn = %lu\n", __func__ , __LINE__, foundLsn);
        fflush(stdout);
#endif

        BufferTag bufferTag;
        bufferTag.rnode = rnode;
        bufferTag.forkNum = (ForkNumber)_forknum;
        bufferTag.blockNum = (BlockNumber)_blknum;
        GetPageFromRocksdb(bufferTag, foundLsn, &targetPage);


        // Set return value from RPC client
        _return.assign(targetPage, BLCKSZ);

//        uint64_t return_lsn = PageGetLSN(targetPage);
#ifdef ENABLE_DEBUG_INFO
        printf("%s %d end, _lsn = %lu, spc = %lu, db = %lu, rel = %lu, fork = %d, blk = %u, return lsn = %lu\n",
               __func__ , __LINE__, _lsn, _reln._spc_node, _reln._db_node, _reln._rel_node, _forknum, _blknum, foundLsn);
        fflush(stdout);
#endif
        free(targetPage);
#ifdef DEBUG_TIMING
        RECORD_TIMING(&start, &end, "ReadPageFromRocksDB");
#endif
//        printf("%s %d end, _lsn = %lu\n", __func__ , __LINE__, _lsn);
//        fflush(stdout);
        return;
...
#ifdef INFO_FUNC_START
        printf("%s start, spc = %lu, db = %lu, rel = %lu, fork = %d\n", __func__ , _reln._spc_node, _reln._db_node,
               _reln._rel_node, _forknum);
        fflush(stdout);
#endif
#ifdef ENABLE_FUNCTION_TIMING
        FunctionTiming functionTiming(const_cast<char *>(__func__));
