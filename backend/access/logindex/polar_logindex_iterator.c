...
//XI: One stack can hold many LSNs.
//      This function will the check whether the stack's head is NULL or full, if it is, allocate a new item and insert it the stack head
//      If the item is available, return the current slot with parameter $idx, and return the item.
...
//XI: Iter one segment's matched LSNs to stack.
static void
                                ...
                                log_seg_id_t seg_id, bool *prev_correct)
{
    ...
    // If this segment is a head node, then get the head node
    // This if-else try to find how many lsn are stored in this segment
    if (head_seg == seg_id)
    {
        // Segment is a union of head node and segment nodes, now we know this segment is a head node
        head = log_index_item_head(table, seg_id);
        size = head->number;
    }
    else
    ...
    for (i = size; i > 0; i--)
    {
        ...
        if (l >= iter->min_lsn && l <= iter->iter_max_lsn)
        {
            ...
            // This function will get a stack segment, and tell you which idx should this lsn be stored.
            stack_item = log_index_tbl_stack_next_item(stack, &stack_idx);
            stack_item->prev_lsn = log_index_get_seg_prev_lsn(table, seg_id, idx);

            // Copy the target idx LSN in this segment to stack_item[stack_idx]
            if (head != NULL)
...
//XI: Iterate all the segments in this table to iter->lsn_stack
static void
...
//        POLAR_LOG_LOGINDEX_TABLE_INFO(table);
    ...
    //XI: Iterate all the segments in this table
    do
    {
...
//            POLAR_LOG_LOGINDEX_TABLE_INFO(table);
        ...
        // XI: This function will push the lsn in this segment to stack
...
// XI: This table should have some lsn in the iter's required range
//XI: If the target page exists in this table, then push the lsn in this table to stack
static void
log_index_push_tbl_lsn(log_index_page_iter_t iter, log_idx_table_data_t *table)
{
    ...
    if (item != NULL)
    {
        ...
        // Here to initialize the iter->iter_prev_lsn
...
log_index_table_in_range(log_index_page_iter_t iter, log_mem_table_t *table)
{
    ...
    // [iter.min, iter.max] [table.min, table.max]
    // Should search a smaller table, so keep going
    if (table->data.min_lsn > iter->max_lsn)
        return false;

    // [table.min, table.max] [iter.min, iter.max]
    // All the table in the following should smaller than this table,
...
//XI: Use this function to fetch the lsn inserted by polar_logindex_create_page_iterator
log_index_lsn_t *
