...
void ApplyOneXLogGetAllRelatedPagesBack(uint64_t lsn, int pageCount, BufferTag* bufferTagList, char* targetPageList) {
    int tid = gettid();
    int replayPid = tid%REPLAY_PROCESS_NUM;
    int gotLock = 0;
    for(int i = 0; i < REPLAY_PROCESS_NUM; i++) {
    ...
    char *requestBuffer = (char*) malloc(1024 + pageCount*sizeof(BufferTag) );
    ...
    msgLen = 4; // $msgLen itself
    msgLen += sizeof(uint64_t); // lsn
    msgLen += sizeof(int); //pageCount, how many mini-xlogs in this xlog
    msgLen += pageCount* sizeof(BufferTag); //basePageList

    msgLen = pg_hton32(msgLen);
    int pageCountCopy = pg_hton32(pageCount);
    memcpy(&requestBuffer[bufferOffset], &msgLen, sizeof(msgLen));
    bufferOffset += sizeof(msgLen);
    memcpy(&requestBuffer[bufferOffset], &lsn, sizeof(uint64_t));
    bufferOffset += sizeof(uint64_t);
    memcpy(&requestBuffer[bufferOffset], &pageCountCopy, sizeof(int));
    bufferOffset += sizeof(int);
    memcpy(&requestBuffer[bufferOffset], bufferTagList, sizeof(BufferTag)*pageCount);
    bufferOffset += sizeof(BufferTag)*pageCount;

    int sendLen = 0;
    while(sendLen < bufferOffset) {
        int writeLen = write(serverPipe[replayPid][1], &requestBuffer[sendLen], bufferOffset-sendLen);
        sendLen+=writeLen;
    }
...
#ifdef DEBUG_INFO
    printf("%s send S request to standalone PG process\n", __func__ );
#endif
    // ------- Read target page from replay process ------
    int recvLen = 0;
    while (recvLen < BLCKSZ*pageCount){
        int readLen = read(computePipe[replayPid][0], &targetPageList[recvLen], BLCKSZ*pageCount - recvLen);
        if (readLen < 0) {
            if (errno == EAGAIN) {
                continue;
            }
...
// UNUSED
void ApplyOneLsnOnSeveralPages(int pageCount, int basePageCount, int basePageExist, char* basePageList, XLogRecord* xlogRecord, char* targetPageList) {
    int tid = gettid();
    int replayPid = tid%REPLAY_PROCESS_NUM;
    int gotLock = 0;
    for(int i = 0; i < REPLAY_PROCESS_NUM; i++) {
    ...
    char *requestBuffer = (char*) malloc(1024+basePageCount*BLCKSZ+xlogRecord->xl_tot_len);
    ...
    msgLen = 4; // $msgLen itself
    msgLen += sizeof(int); //pageCount, how many mini-xlogs in this xlog
    msgLen += sizeof(int); //basePageCount, how many base pages in this xlog
    msgLen += sizeof(int); //basePageExist, whether base page exist for every mini-xlog
    msgLen += basePageCount*BLCKSZ; //basePageList
    msgLen += xlogRecord->xl_tot_len; //xlogRecord

    msgLen = pg_hton32(msgLen);
    memcpy(&requestBuffer[bufferOffset], &msgLen, sizeof(msgLen));
    bufferOffset += sizeof(msgLen);
    memcpy(&requestBuffer[bufferOffset], &pageCount, sizeof(int));
    bufferOffset += sizeof(int);
    memcpy(&requestBuffer[bufferOffset], &basePageCount, sizeof(int));
    bufferOffset += sizeof(int);
    memcpy(&requestBuffer[bufferOffset], &basePageExist, sizeof(int));
    bufferOffset += sizeof(int);
    memcpy(&requestBuffer[bufferOffset], basePageList, basePageCount*BLCKSZ);
    bufferOffset += basePageCount*BLCKSZ;
    memcpy(&requestBuffer[bufferOffset], xlogRecord, xlogRecord->xl_tot_len);
    bufferOffset += xlogRecord->xl_tot_len;
...
extern XLogRecPtr XLogReplayUpto;
void* RecordReplayProgress() {
    struct timeval now;
    while(1) {
        usleep(100*1000);
        gettimeofday(&now, NULL);
        printf("RecordReplayProgress, parsed=%ld, replayed=%ld, sec=%lu, us=%lu\n", XLogParseUpto, XLogReplayUpto, now.tv_sec, now.tv_usec);
        fflush(stdout);
    }
}
...
//    StartupPid = StartChildProcess(StartupProcess);
    ...
    pthread_create(&XlogStartupTid3, NULL, (void*)StartupXLOG2, NULL);
    pthread_t tempTid;
