...
void ApplyOneXLogGetAllRelatedPagesBack(uint64_t lsn, int pageCount, BufferTag* bufferTagList, XLogRecord* xlogRecord, char* targetPageList) {
    int tid = gettid();
    int replayPid = tid%REPLAY_PROCESS_NUM;
    int gotLock = 0;
    for(int i = 0; i < REPLAY_PROCESS_NUM; i++) {
    ...
    char *requestBuffer = (char*) malloc(1024 + pageCount*sizeof(BufferTag) + xlogRecord->xl_tot_len);
...
void* RecordReplayProgress() {
    ...
    while(1) {
        usleep(100*1000);
        gettimeofday(&now, NULL);
        printf("RecordReplayProgress, replayed=%ld, sec=%lu, us=%lu\n", XLogParseUpto, now.tv_sec, now.tv_usec);
        fflush(stdout);
    }
}
